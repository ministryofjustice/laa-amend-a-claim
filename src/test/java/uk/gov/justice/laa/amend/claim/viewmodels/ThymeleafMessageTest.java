...
class ThymeleafMessageTest {
    ...
    void resolve_shouldResolvePlainStringParams() {
        when(messages.msgWithParams(eq("key"), any()))
            .thenReturn("resolved");

        ThymeleafMessage message =
            new ThymeleafMessage("key", "param1", "param2");
    ...
    void resolve_shouldResolveNestedThymeleafMessages() {
        when(messages.msgWithParams(eq("inner.key"), any()))
            .thenReturn("innerValue");

        when(messages.msgWithParams(eq("outer.key"), any()))
            .thenReturn("outerValue");

        ThymeleafMessage inner =
            new ThymeleafMessage("inner.key");

        ThymeleafMessage outer =
            new ThymeleafMessage("outer.key", inner);
    ...
    void resolve_shouldHandleMixedParamTypes() {
        ...
        ThymeleafMessage message =
            new ThymeleafMessage("key", "text", inner, 42, true, null);
        ...
        assertThat(captor.getValue())
            .containsExactly("text", "inner", "42", "true", "");
    }
}
