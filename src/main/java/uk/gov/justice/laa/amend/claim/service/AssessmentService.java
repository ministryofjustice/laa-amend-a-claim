...
public class AssessmentService {
  ...
  /**
   * Retrieves the latest assessment for a claim and updates {@link ClaimDetails} based on VOID or
   * ESCAPE_CASE_ASSESSMENT rules. VOID: the latest assessment must be VOID, and previous
   * escape‑case values are applied if available. Non‑VOID: the latest assessment must be
   * ESCAPE_CASE_ASSESSMENT; null types are treated as escape‑case during migration.
   *
  ...
  public ClaimDetails getLatestAssessmentByClaim(ClaimDetails claimDetails) {
    ...
    var latestEscapeCaseAssessment =
        assessments.stream()
  ...
  /**
   * Validates the latest assessment for VOID and non‑VOID claims. - VOID - latest assessment must
   * be VOID - Non-VOID - latest assessment must ESCAPE_CASE_ASSESSMENT Returns the original
   * assessment list if validation succeeds.
   *
  ...
  private List<AssessmentGet> validateAndGetAssessments(
    ...
    log.info(
        ...
        claimDetails.getClaimId());
    // Currently treats null as Escape case assessments until the migration completes for VOID
    // assessments.
    // This should be to just retrieve assessments from AssessmentResults after migration.
    List<AssessmentGet> assessments =
        assessmentResults.getAssessments().stream()
            .filter(Objects::nonNull)
