  ...
  protected BulkUploadValidationOutcome validateRows(List<BulkUploadCivilClaim> rows) {
    ...
    var claimsByOfficeCodeAndUfn =
        apiClaimResponseObjects.stream()
            .collect(
                groupingBy(claim -> Pair.of(claim.getOfficeCode(), claim.getUniqueFileNumber())));
    for (var row : rows) {
      var claimResponsesForRow =
          claimsByOfficeCodeAndUfn.get(Pair.of(row.getOfficeCode(), row.getUfn()));
