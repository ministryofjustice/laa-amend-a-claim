...
public interface AssessmentMapper {
  ...
  CrimeClaimDetails mapToCrimeClaim(
      AssessmentInfo assessmentInfo, @MappingTarget CrimeClaimDetails claimDetails);
  ...
  default void mapToCivilClaimAfterMapping(
      AssessmentInfo source, @MappingTarget CivilClaimDetails target) {
    map(source, target.getJrFormFillingCost(), AssessmentInfo::jrFormFillingAmount);
    map(
        source,
        target.getDetentionTravelWaitingCosts(),
        AssessmentInfo::detentionTravelAndWaitingCostsAmount);
  ...
  default void mapToCrimeClaimAfterMapping(
      AssessmentInfo source, @MappingTarget CrimeClaimDetails target) {
  ...
  @Mapping(
      target = "lastAssessmentOutcome",
      source = "assessmentOutcome",
      qualifiedByName = "mapToOutcome")
  ...
  @Mapping(
      target = "assessmentOutcome",
      source = "assessmentOutcome",
      qualifiedByName = "mapToOutcome")
  @Mapping(target = "assessmentReason", source = "assessmentReason")
  @Mapping(
      target = "lastAssessment",
      source = "assessmentGet",
      qualifiedByName = "toAssessmentInfo")
