...
import uk.gov.justice.laa.payments.amend.forms.SearchForm;
import uk.gov.justice.laa.payments.amend.forms.annotations.ValidSubmissionDate;
import uk.gov.justice.laa.payments.amend.forms.errors.FieldError;
import uk.gov.justice.laa.payments.amend.forms.errors.FieldErrorType;

public class DateValidator extends Validator
    implements ConstraintValidator<ValidSubmissionDate, SearchForm> {
  ...
  private Either<FieldError, Integer> validateMonth(SearchForm form) {
    if (isBlank(form.getSubmissionDateMonth())) {
      return Either.left(
          new FieldError(FieldErrorType.REQUIRED, "{index.submissionDate.month.error.required}"));
    }
    ...
    if (month == null) {
      return Either.left(
          new FieldError(FieldErrorType.INVALID, "{index.submissionDate.month.error.invalid}"));
    }

    if (month < 1 || month > 12) {
      return Either.left(
          new FieldError(FieldErrorType.INVALID, "{index.submissionDate.month.error.range}"));
    }
  ...
  private Either<FieldError, Integer> validateYear(SearchForm form) {
    if (isBlank(form.getSubmissionDateYear())) {
      return Either.left(
          new FieldError(FieldErrorType.REQUIRED, "{index.submissionDate.year.error.required}"));
    }
    ...
    if (year == null) {
      return Either.left(
          new FieldError(FieldErrorType.INVALID, "{index.submissionDate.year.error.invalid}"));
    }
