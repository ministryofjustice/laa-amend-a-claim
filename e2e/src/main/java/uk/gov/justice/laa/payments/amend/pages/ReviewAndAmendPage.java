...
import static uk.gov.justice.laa.payments.amend.helpers.PageHelper.cardByTitle;
import static uk.gov.justice.laa.payments.amend.helpers.PageHelper.tableByCard;
...
public class ReviewAndAmendPage extends LaaErrorSummaryPage {
  ...
  public ReviewAndAmendPage(Page page) {
    ...
    this.saveButton =
        page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Submit adjustments"));
  ...
  private void clickChangeInRow(String itemName, Locator locator) {
    rowByItemName(itemName, locator).locator("a.govuk-link:has-text('Change')").click();
  }
  ...
  public void clickContingencyAssessment() {
    clickChangeInRow(
        "Was this claim assessed as part of the contingency process?", assessmentTable);
  }
  ...
  private void assertTableHasHeaders(Locator table, String... headers) {
    ...
    for (String header : headers) {
      assertThat(
              table.getByRole(
                  AriaRole.COLUMNHEADER, new Locator.GetByRoleOptions().setName(header)))
          .isVisible();
    }
  }
